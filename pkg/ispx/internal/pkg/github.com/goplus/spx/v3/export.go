/*
 * Copyright (c) 2021 The XGo Authors (xgo.dev). All rights reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

// export by github.com/goplus/ixgo/cmd/qexp

package spx

import (
	q "github.com/goplus/spx/v3"

	"github.com/goplus/ixgo/alias"
	"go/constant"
	"go/token"
	"reflect"

	_ "embed"
	"github.com/goplus/ixgo"
	"github.com/goplus/ixgo/xgobuild/typesdata"
	event "github.com/goplus/spx/v3/internal/core/event"
	engine "github.com/goplus/spx/v3/pkg/spx/pkg/engine"
)

//go:embed export.types
var exportTypesData []byte

func init() {
	var (
		alias_Seconds = &alias.Alias{Typ: "Seconds"}
	)
	ixgo.RegisterPackage(&ixgo.Package{
		Name: "spx",
		Path: "github.com/goplus/spx/v3",
		Deps: map[string]string{
			"errors":                            "errors",
			"flag":                              "flag",
			"fmt":                               "fmt",
			"github.com/goplus/spbase/mathf":    "mathf",
			"github.com/goplus/spx/v3/fs":       "fs",
			"github.com/goplus/spx/v3/fs/asset": "asset",
			"github.com/goplus/spx/v3/fs/zip":   "zip",
			"github.com/goplus/spx/v3/internal/animation":       "animation",
			"github.com/goplus/spx/v3/internal/assets":          "assets",
			"github.com/goplus/spx/v3/internal/audio":           "audio",
			"github.com/goplus/spx/v3/internal/base/collision":  "collision",
			"github.com/goplus/spx/v3/internal/base/defaults":   "defaults",
			"github.com/goplus/spx/v3/internal/base/sliceutil":  "sliceutil",
			"github.com/goplus/spx/v3/internal/core/event":      "event",
			"github.com/goplus/spx/v3/internal/core/project":    "project",
			"github.com/goplus/spx/v3/internal/core/runtime":    "runtime",
			"github.com/goplus/spx/v3/internal/core/state":      "state",
			"github.com/goplus/spx/v3/internal/coroutine":       "coroutine",
			"github.com/goplus/spx/v3/internal/debug":           "debug",
			"github.com/goplus/spx/v3/internal/engine":          "engine",
			"github.com/goplus/spx/v3/internal/engine/platform": "platform",
			"github.com/goplus/spx/v3/internal/engine/profiler": "profiler",
			"github.com/goplus/spx/v3/internal/enginewrap":      "enginewrap",
			"github.com/goplus/spx/v3/internal/input":           "input",
			"github.com/goplus/spx/v3/internal/input/keycode":   "keycode",
			"github.com/goplus/spx/v3/internal/log":             "log",
			"github.com/goplus/spx/v3/internal/scratch":         "scratch",
			"github.com/goplus/spx/v3/internal/tilemap":         "tilemap",
			"github.com/goplus/spx/v3/internal/time":            "time",
			"github.com/goplus/spx/v3/internal/tools":           "tools",
			"github.com/goplus/spx/v3/internal/ui":              "ui",
			"github.com/goplus/spx/v3/pkg/spx":                  "spx",
			"github.com/goplus/spx/v3/pkg/spx/pkg/engine":       "engine",
			"maps":        "maps",
			"math":        "math",
			"math/rand":   "rand",
			"os":          "os",
			"path":        "path",
			"reflect":     "reflect",
			"runtime":     "runtime",
			"slices":      "slices",
			"sort":        "sort",
			"strconv":     "strconv",
			"strings":     "strings",
			"sync":        "sync",
			"sync/atomic": "atomic",
			"syscall":     "syscall",
			"time":        "time",
			"unsafe":      "unsafe",
		},
		Interfaces: map[string]reflect.Type{
			"Camera":      reflect.TypeOf((*q.Camera)(nil)).Elem(),
			"Gamer":       reflect.TypeOf((*q.Gamer)(nil)).Elem(),
			"Shape":       reflect.TypeOf((*q.Shape)(nil)).Elem(),
			"ShapeGetter": reflect.TypeOf((*q.ShapeGetter)(nil)).Elem(),
			"Sprite":      reflect.TypeOf((*q.Sprite)(nil)).Elem(),
			"Widget":      reflect.TypeOf((*q.Widget)(nil)).Elem(),
		},
		NamedTypes: map[string]reflect.Type{
			"Color":                   reflect.TypeOf((*q.Color)(nil)).Elem(),
			"EffectKind":              reflect.TypeOf((*q.EffectKind)(nil)).Elem(),
			"Game":                    reflect.TypeOf((*q.Game)(nil)).Elem(),
			"InputSessionOptions":     reflect.TypeOf((*q.InputSessionOptions)(nil)).Elem(),
			"InputSessionPreparation": reflect.TypeOf((*q.InputSessionPreparation)(nil)).Elem(),
			"List":                    reflect.TypeOf((*q.List)(nil)).Elem(),
			"Monitor":                 reflect.TypeOf((*q.Monitor)(nil)).Elem(),
			"MotionOptions":           reflect.TypeOf((*q.MotionOptions)(nil)).Elem(),
			"PenColorParam":           reflect.TypeOf((*q.PenColorParam)(nil)).Elem(),
			"RotationStyle":           reflect.TypeOf((*q.RotationStyle)(nil)).Elem(),
			"SoundEffectKind":         reflect.TypeOf((*q.SoundEffectKind)(nil)).Elem(),
			"SpriteImpl":              reflect.TypeOf((*q.SpriteImpl)(nil)).Elem(),
			"Value":                   reflect.TypeOf((*q.Value)(nil)).Elem(),
		},
		AliasTypes: map[string]reflect.Type{
			"BackdropName":          reflect.TypeOf((*q.BackdropName)(nil)).Elem(),
			"ColliderShapeType":     reflect.TypeOf((*q.ColliderShapeType)(nil)).Elem(),
			"Config":                reflect.TypeOf((*q.Config)(nil)).Elem(),
			"DecoratorJSON":         reflect.TypeOf((*q.DecoratorJSON)(nil)).Elem(),
			"Direction":             reflect.TypeOf((*q.Direction)(nil)).Elem(),
			"IEventSinks":           reflect.TypeOf((*q.IEventSinks)(nil)).Elem(),
			"InputReplay":           reflect.TypeOf((*q.InputReplay)(nil)).Elem(),
			"InputReplayFrame":      reflect.TypeOf((*q.InputReplayFrame)(nil)).Elem(),
			"InputReplayKeyEvent":   reflect.TypeOf((*q.InputReplayKeyEvent)(nil)).Elem(),
			"InputReplayMouse":      reflect.TypeOf((*q.InputReplayMouse)(nil)).Elem(),
			"InputReplayMouseEvent": reflect.TypeOf((*q.InputReplayMouseEvent)(nil)).Elem(),
			"InputReplayState":      reflect.TypeOf((*q.InputReplayState)(nil)).Elem(),
			"InputSessionMode":      reflect.TypeOf((*q.InputSessionMode)(nil)).Elem(),
			"InputSessionPhase":     reflect.TypeOf((*q.InputSessionPhase)(nil)).Elem(),
			"InputSessionStatus":    reflect.TypeOf((*q.InputSessionStatus)(nil)).Elem(),
			"Key":                   reflect.TypeOf((*q.Key)(nil)).Elem(),
			"MsgName":               reflect.TypeOf((*q.MsgName)(nil)).Elem(),
			"PhysicsMode":           reflect.TypeOf((*q.PhysicsMode)(nil)).Elem(),
			"Pos":                   reflect.TypeOf((*q.Pos)(nil)).Elem(),
			"PropertyName":          reflect.TypeOf((*q.PropertyName)(nil)).Elem(),
			"Seconds":               reflect.TypeOf((*q.Seconds)(nil)).Elem(),
			"SoundName":             reflect.TypeOf((*q.SoundName)(nil)).Elem(),
			"Speed":                 reflect.TypeOf((*q.Speed)(nil)).Elem(),
			"SpriteAnimationName":   reflect.TypeOf((*q.SpriteAnimationName)(nil)).Elem(),
			"SpriteCostumeName":     reflect.TypeOf((*q.SpriteCostumeName)(nil)).Elem(),
			"SpriteName":            reflect.TypeOf((*q.SpriteName)(nil)).Elem(),
			"StopKind":              reflect.TypeOf((*q.StopKind)(nil)).Elem(),
			"Target":                reflect.TypeOf((*q.Target)(nil)).Elem(),
			"VarName":               reflect.TypeOf((*q.VarName)(nil)).Elem(),
			"WidgetName":            reflect.TypeOf((*q.WidgetName)(nil)).Elem(),
		},
		Vars: map[string]reflect.Value{
			"ErrInputSessionActive":       reflect.ValueOf(&q.ErrInputSessionActive),
			"ErrInputSessionNotRecording": reflect.ValueOf(&q.ErrInputSessionNotRecording),
		},
		Funcs: map[string]reflect.Value{
			"AtFrame":                    reflect.ValueOf(q.AtFrame),
			"CharAt":                     reflect.ValueOf(q.CharAt),
			"Compare":                    reflect.ValueOf(q.Compare),
			"Contains":                   reflect.ValueOf(q.Contains),
			"CurrentFrame":               reflect.ValueOf(q.CurrentFrame),
			"DecodeInputReplay":          reflect.ValueOf(q.DecodeInputReplay),
			"DeltaTime":                  reflect.ValueOf(q.DeltaTime),
			"EncodeInputReplay":          reflect.ValueOf(q.EncodeInputReplay),
			"Equal":                      reflect.ValueOf(q.Equal),
			"Exit__0":                    reflect.ValueOf(q.Exit__0),
			"Exit__1":                    reflect.ValueOf(q.Exit__1),
			"FinishInputRecording":       reflect.ValueOf(q.FinishInputRecording),
			"FinishInputRecordingJSON":   reflect.ValueOf(q.FinishInputRecordingJSON),
			"FloorMod":                   reflect.ValueOf(q.FloorMod),
			"Forever":                    reflect.ValueOf(q.Forever),
			"GetInputSessionStatus":      reflect.ValueOf(q.GetInputSessionStatus),
			"GetWidget":                  reflect.ValueOf(q.GetWidget),
			"HSB":                        reflect.ValueOf(q.HSB),
			"HSBA":                       reflect.ValueOf(q.HSBA),
			"Iround":                     reflect.ValueOf(q.Iround),
			"IsRunWithoutScreenRefresh":  reflect.ValueOf(q.IsRunWithoutScreenRefresh),
			"KeyFromString":              reflect.ValueOf(q.KeyFromString),
			"NewColor":                   reflect.ValueOf(q.NewColor),
			"NewList":                    reflect.ValueOf(q.NewList),
			"NewValue":                   reflect.ValueOf(q.NewValue),
			"PenColorParamFromString":    reflect.ValueOf(q.PenColorParamFromString),
			"PrepareInputRecording":      reflect.ValueOf(q.PrepareInputRecording),
			"PrepareInputReplay":         reflect.ValueOf(q.PrepareInputReplay),
			"Procedure":                  reflect.ValueOf(q.Procedure),
			"Rand__0":                    reflect.ValueOf(q.Rand__0),
			"Rand__1":                    reflect.ValueOf(q.Rand__1),
			"Repeat":                     reflect.ValueOf(q.Repeat),
			"RepeatUntil":                reflect.ValueOf(q.RepeatUntil),
			"ResetRandomSeed":            reflect.ValueOf(q.ResetRandomSeed),
			"Sched":                      reflect.ValueOf(q.Sched),
			"SchedNow":                   reflect.ValueOf(q.SchedNow),
			"ScratchListIndex":           reflect.ValueOf(q.ScratchListIndex),
			"SetDebug":                   reflect.ValueOf(q.SetDebug),
			"SetRandomSeed":              reflect.ValueOf(q.SetRandomSeed),
			"SetRunWithoutScreenRefresh": reflect.ValueOf(q.SetRunWithoutScreenRefresh),
			"Snapshot":                   reflect.ValueOf(q.Snapshot),
			"TimeSinceLevelLoad":         reflect.ValueOf(q.TimeSinceLevelLoad),
			"WaitUntil":                  reflect.ValueOf(q.WaitUntil),
			"Warp":                       reflect.ValueOf(q.Warp),
			"XGot_Game_Main":             reflect.ValueOf(q.XGot_Game_Main),
			"XGot_Game_Reload":           reflect.ValueOf(q.XGot_Game_Reload),
		},
		TypedConsts: map[string]ixgo.TypedConst{
			"AllOtherScripts":            {Typ: reflect.TypeOf(q.AllOtherScripts), Value: constant.MakeInt64(int64(q.AllOtherScripts))},
			"AllSprites":                 {Typ: reflect.TypeOf(q.AllSprites), Value: constant.MakeInt64(int64(q.AllSprites))},
			"AllStop":                    {Typ: reflect.TypeOf(q.AllStop), Value: constant.MakeInt64(int64(q.AllStop))},
			"AnimChannelFrame":           {Typ: reflect.TypeOf(q.AnimChannelFrame), Value: constant.MakeString(string(q.AnimChannelFrame))},
			"AnimChannelGlide":           {Typ: reflect.TypeOf(q.AnimChannelGlide), Value: constant.MakeString(string(q.AnimChannelGlide))},
			"AnimChannelMove":            {Typ: reflect.TypeOf(q.AnimChannelMove), Value: constant.MakeString(string(q.AnimChannelMove))},
			"AnimChannelTurn":            {Typ: reflect.TypeOf(q.AnimChannelTurn), Value: constant.MakeString(string(q.AnimChannelTurn))},
			"Back":                       {Typ: reflect.TypeOf(q.Back), Value: constant.MakeInt64(int64(q.Back))},
			"Backward":                   {Typ: reflect.TypeOf(q.Backward), Value: constant.MakeInt64(int64(q.Backward))},
			"BrightnessEffect":           {Typ: reflect.TypeOf(q.BrightnessEffect), Value: constant.MakeInt64(int64(q.BrightnessEffect))},
			"CapsuleCollider":            {Typ: reflect.TypeOf(q.CapsuleCollider), Value: constant.MakeInt64(int64(q.CapsuleCollider))},
			"CircleCollider":             {Typ: reflect.TypeOf(q.CircleCollider), Value: constant.MakeInt64(int64(q.CircleCollider))},
			"ColorEffect":                {Typ: reflect.TypeOf(q.ColorEffect), Value: constant.MakeInt64(int64(q.ColorEffect))},
			"DbgFlagAll":                 {Typ: reflect.TypeOf(q.DbgFlagAll), Value: constant.MakeInt64(int64(q.DbgFlagAll))},
			"DbgFlagEvent":               {Typ: reflect.TypeOf(q.DbgFlagEvent), Value: constant.MakeInt64(int64(q.DbgFlagEvent))},
			"DbgFlagInstr":               {Typ: reflect.TypeOf(q.DbgFlagInstr), Value: constant.MakeInt64(int64(q.DbgFlagInstr))},
			"DbgFlagLoad":                {Typ: reflect.TypeOf(q.DbgFlagLoad), Value: constant.MakeInt64(int64(q.DbgFlagLoad))},
			"DbgFlagPerf":                {Typ: reflect.TypeOf(q.DbgFlagPerf), Value: constant.MakeInt64(int64(q.DbgFlagPerf))},
			"Down":                       {Typ: reflect.TypeOf(q.Down), Value: constant.MakeFromLiteral("1.8e+2", token.FLOAT, 0)},
			"DynamicPhysics":             {Typ: reflect.TypeOf(q.DynamicPhysics), Value: constant.MakeInt64(int64(q.DynamicPhysics))},
			"Edge":                       {Typ: reflect.TypeOf(q.Edge), Value: constant.MakeInt64(int64(q.Edge))},
			"EdgeBottom":                 {Typ: reflect.TypeOf(q.EdgeBottom), Value: constant.MakeInt64(int64(q.EdgeBottom))},
			"EdgeLeft":                   {Typ: reflect.TypeOf(q.EdgeLeft), Value: constant.MakeInt64(int64(q.EdgeLeft))},
			"EdgeRight":                  {Typ: reflect.TypeOf(q.EdgeRight), Value: constant.MakeInt64(int64(q.EdgeRight))},
			"EdgeTop":                    {Typ: reflect.TypeOf(q.EdgeTop), Value: constant.MakeInt64(int64(q.EdgeTop))},
			"FishEyeEffect":              {Typ: reflect.TypeOf(q.FishEyeEffect), Value: constant.MakeInt64(int64(q.FishEyeEffect))},
			"Forward":                    {Typ: reflect.TypeOf(q.Forward), Value: constant.MakeInt64(int64(q.Forward))},
			"Front":                      {Typ: reflect.TypeOf(q.Front), Value: constant.MakeInt64(int64(q.Front))},
			"GhostEffect":                {Typ: reflect.TypeOf(q.GhostEffect), Value: constant.MakeInt64(int64(q.GhostEffect))},
			"InputSessionModeIdle":       {Typ: reflect.TypeOf(q.InputSessionModeIdle), Value: constant.MakeString(string(q.InputSessionModeIdle))},
			"InputSessionModeRecording":  {Typ: reflect.TypeOf(q.InputSessionModeRecording), Value: constant.MakeString(string(q.InputSessionModeRecording))},
			"InputSessionModeReplaying":  {Typ: reflect.TypeOf(q.InputSessionModeReplaying), Value: constant.MakeString(string(q.InputSessionModeReplaying))},
			"InputSessionPhaseAborted":   {Typ: reflect.TypeOf(q.InputSessionPhaseAborted), Value: constant.MakeString(string(q.InputSessionPhaseAborted))},
			"InputSessionPhaseCompleted": {Typ: reflect.TypeOf(q.InputSessionPhaseCompleted), Value: constant.MakeString(string(q.InputSessionPhaseCompleted))},
			"InputSessionPhaseFinishing": {Typ: reflect.TypeOf(q.InputSessionPhaseFinishing), Value: constant.MakeString(string(q.InputSessionPhaseFinishing))},
			"InputSessionPhasePrepared":  {Typ: reflect.TypeOf(q.InputSessionPhasePrepared), Value: constant.MakeString(string(q.InputSessionPhasePrepared))},
			"InputSessionPhaseRunning":   {Typ: reflect.TypeOf(q.InputSessionPhaseRunning), Value: constant.MakeString(string(q.InputSessionPhaseRunning))},
			"Invalid":                    {Typ: reflect.TypeOf(q.Invalid), Value: constant.MakeInt64(int64(q.Invalid))},
			"Key0":                       {Typ: reflect.TypeOf(q.Key0), Value: constant.MakeInt64(int64(q.Key0))},
			"Key1":                       {Typ: reflect.TypeOf(q.Key1), Value: constant.MakeInt64(int64(q.Key1))},
			"Key2":                       {Typ: reflect.TypeOf(q.Key2), Value: constant.MakeInt64(int64(q.Key2))},
			"Key3":                       {Typ: reflect.TypeOf(q.Key3), Value: constant.MakeInt64(int64(q.Key3))},
			"Key4":                       {Typ: reflect.TypeOf(q.Key4), Value: constant.MakeInt64(int64(q.Key4))},
			"Key5":                       {Typ: reflect.TypeOf(q.Key5), Value: constant.MakeInt64(int64(q.Key5))},
			"Key6":                       {Typ: reflect.TypeOf(q.Key6), Value: constant.MakeInt64(int64(q.Key6))},
			"Key7":                       {Typ: reflect.TypeOf(q.Key7), Value: constant.MakeInt64(int64(q.Key7))},
			"Key8":                       {Typ: reflect.TypeOf(q.Key8), Value: constant.MakeInt64(int64(q.Key8))},
			"Key9":                       {Typ: reflect.TypeOf(q.Key9), Value: constant.MakeInt64(int64(q.Key9))},
			"KeyA":                       {Typ: reflect.TypeOf(q.KeyA), Value: constant.MakeInt64(int64(q.KeyA))},
			"KeyAlt":                     {Typ: reflect.TypeOf(q.KeyAlt), Value: constant.MakeInt64(int64(q.KeyAlt))},
			"KeyAny":                     {Typ: reflect.TypeOf(q.KeyAny), Value: constant.MakeInt64(int64(q.KeyAny))},
			"KeyApostrophe":              {Typ: reflect.TypeOf(q.KeyApostrophe), Value: constant.MakeInt64(int64(q.KeyApostrophe))},
			"KeyB":                       {Typ: reflect.TypeOf(q.KeyB), Value: constant.MakeInt64(int64(q.KeyB))},
			"KeyBackslash":               {Typ: reflect.TypeOf(q.KeyBackslash), Value: constant.MakeInt64(int64(q.KeyBackslash))},
			"KeyBackspace":               {Typ: reflect.TypeOf(q.KeyBackspace), Value: constant.MakeInt64(int64(q.KeyBackspace))},
			"KeyC":                       {Typ: reflect.TypeOf(q.KeyC), Value: constant.MakeInt64(int64(q.KeyC))},
			"KeyCapsLock":                {Typ: reflect.TypeOf(q.KeyCapsLock), Value: constant.MakeInt64(int64(q.KeyCapsLock))},
			"KeyComma":                   {Typ: reflect.TypeOf(q.KeyComma), Value: constant.MakeInt64(int64(q.KeyComma))},
			"KeyControl":                 {Typ: reflect.TypeOf(q.KeyControl), Value: constant.MakeInt64(int64(q.KeyControl))},
			"KeyD":                       {Typ: reflect.TypeOf(q.KeyD), Value: constant.MakeInt64(int64(q.KeyD))},
			"KeyDelete":                  {Typ: reflect.TypeOf(q.KeyDelete), Value: constant.MakeInt64(int64(q.KeyDelete))},
			"KeyDown":                    {Typ: reflect.TypeOf(q.KeyDown), Value: constant.MakeInt64(int64(q.KeyDown))},
			"KeyE":                       {Typ: reflect.TypeOf(q.KeyE), Value: constant.MakeInt64(int64(q.KeyE))},
			"KeyEnd":                     {Typ: reflect.TypeOf(q.KeyEnd), Value: constant.MakeInt64(int64(q.KeyEnd))},
			"KeyEnter":                   {Typ: reflect.TypeOf(q.KeyEnter), Value: constant.MakeInt64(int64(q.KeyEnter))},
			"KeyEqual":                   {Typ: reflect.TypeOf(q.KeyEqual), Value: constant.MakeInt64(int64(q.KeyEqual))},
			"KeyEscape":                  {Typ: reflect.TypeOf(q.KeyEscape), Value: constant.MakeInt64(int64(q.KeyEscape))},
			"KeyExclam":                  {Typ: reflect.TypeOf(q.KeyExclam), Value: constant.MakeInt64(int64(q.KeyExclam))},
			"KeyF":                       {Typ: reflect.TypeOf(q.KeyF), Value: constant.MakeInt64(int64(q.KeyF))},
			"KeyF1":                      {Typ: reflect.TypeOf(q.KeyF1), Value: constant.MakeInt64(int64(q.KeyF1))},
			"KeyF10":                     {Typ: reflect.TypeOf(q.KeyF10), Value: constant.MakeInt64(int64(q.KeyF10))},
			"KeyF11":                     {Typ: reflect.TypeOf(q.KeyF11), Value: constant.MakeInt64(int64(q.KeyF11))},
			"KeyF12":                     {Typ: reflect.TypeOf(q.KeyF12), Value: constant.MakeInt64(int64(q.KeyF12))},
			"KeyF2":                      {Typ: reflect.TypeOf(q.KeyF2), Value: constant.MakeInt64(int64(q.KeyF2))},
			"KeyF3":                      {Typ: reflect.TypeOf(q.KeyF3), Value: constant.MakeInt64(int64(q.KeyF3))},
			"KeyF4":                      {Typ: reflect.TypeOf(q.KeyF4), Value: constant.MakeInt64(int64(q.KeyF4))},
			"KeyF5":                      {Typ: reflect.TypeOf(q.KeyF5), Value: constant.MakeInt64(int64(q.KeyF5))},
			"KeyF6":                      {Typ: reflect.TypeOf(q.KeyF6), Value: constant.MakeInt64(int64(q.KeyF6))},
			"KeyF7":                      {Typ: reflect.TypeOf(q.KeyF7), Value: constant.MakeInt64(int64(q.KeyF7))},
			"KeyF8":                      {Typ: reflect.TypeOf(q.KeyF8), Value: constant.MakeInt64(int64(q.KeyF8))},
			"KeyF9":                      {Typ: reflect.TypeOf(q.KeyF9), Value: constant.MakeInt64(int64(q.KeyF9))},
			"KeyG":                       {Typ: reflect.TypeOf(q.KeyG), Value: constant.MakeInt64(int64(q.KeyG))},
			"KeyGraveAccent":             {Typ: reflect.TypeOf(q.KeyGraveAccent), Value: constant.MakeInt64(int64(q.KeyGraveAccent))},
			"KeyH":                       {Typ: reflect.TypeOf(q.KeyH), Value: constant.MakeInt64(int64(q.KeyH))},
			"KeyHome":                    {Typ: reflect.TypeOf(q.KeyHome), Value: constant.MakeInt64(int64(q.KeyHome))},
			"KeyI":                       {Typ: reflect.TypeOf(q.KeyI), Value: constant.MakeInt64(int64(q.KeyI))},
			"KeyInsert":                  {Typ: reflect.TypeOf(q.KeyInsert), Value: constant.MakeInt64(int64(q.KeyInsert))},
			"KeyJ":                       {Typ: reflect.TypeOf(q.KeyJ), Value: constant.MakeInt64(int64(q.KeyJ))},
			"KeyK":                       {Typ: reflect.TypeOf(q.KeyK), Value: constant.MakeInt64(int64(q.KeyK))},
			"KeyKP0":                     {Typ: reflect.TypeOf(q.KeyKP0), Value: constant.MakeInt64(int64(q.KeyKP0))},
			"KeyKP1":                     {Typ: reflect.TypeOf(q.KeyKP1), Value: constant.MakeInt64(int64(q.KeyKP1))},
			"KeyKP2":                     {Typ: reflect.TypeOf(q.KeyKP2), Value: constant.MakeInt64(int64(q.KeyKP2))},
			"KeyKP3":                     {Typ: reflect.TypeOf(q.KeyKP3), Value: constant.MakeInt64(int64(q.KeyKP3))},
			"KeyKP4":                     {Typ: reflect.TypeOf(q.KeyKP4), Value: constant.MakeInt64(int64(q.KeyKP4))},
			"KeyKP5":                     {Typ: reflect.TypeOf(q.KeyKP5), Value: constant.MakeInt64(int64(q.KeyKP5))},
			"KeyKP6":                     {Typ: reflect.TypeOf(q.KeyKP6), Value: constant.MakeInt64(int64(q.KeyKP6))},
			"KeyKP7":                     {Typ: reflect.TypeOf(q.KeyKP7), Value: constant.MakeInt64(int64(q.KeyKP7))},
			"KeyKP8":                     {Typ: reflect.TypeOf(q.KeyKP8), Value: constant.MakeInt64(int64(q.KeyKP8))},
			"KeyKP9":                     {Typ: reflect.TypeOf(q.KeyKP9), Value: constant.MakeInt64(int64(q.KeyKP9))},
			"KeyKPDecimal":               {Typ: reflect.TypeOf(q.KeyKPDecimal), Value: constant.MakeInt64(int64(q.KeyKPDecimal))},
			"KeyKPDivide":                {Typ: reflect.TypeOf(q.KeyKPDivide), Value: constant.MakeInt64(int64(q.KeyKPDivide))},
			"KeyKPEnter":                 {Typ: reflect.TypeOf(q.KeyKPEnter), Value: constant.MakeInt64(int64(q.KeyKPEnter))},
			"KeyKPEqual":                 {Typ: reflect.TypeOf(q.KeyKPEqual), Value: constant.MakeInt64(int64(q.KeyKPEqual))},
			"KeyKPMultiply":              {Typ: reflect.TypeOf(q.KeyKPMultiply), Value: constant.MakeInt64(int64(q.KeyKPMultiply))},
			"KeyKPSubtract":              {Typ: reflect.TypeOf(q.KeyKPSubtract), Value: constant.MakeInt64(int64(q.KeyKPSubtract))},
			"KeyL":                       {Typ: reflect.TypeOf(q.KeyL), Value: constant.MakeInt64(int64(q.KeyL))},
			"KeyLeft":                    {Typ: reflect.TypeOf(q.KeyLeft), Value: constant.MakeInt64(int64(q.KeyLeft))},
			"KeyLeftBracket":             {Typ: reflect.TypeOf(q.KeyLeftBracket), Value: constant.MakeInt64(int64(q.KeyLeftBracket))},
			"KeyM":                       {Typ: reflect.TypeOf(q.KeyM), Value: constant.MakeInt64(int64(q.KeyM))},
			"KeyMax":                     {Typ: reflect.TypeOf(q.KeyMax), Value: constant.MakeInt64(int64(q.KeyMax))},
			"KeyMenu":                    {Typ: reflect.TypeOf(q.KeyMenu), Value: constant.MakeInt64(int64(q.KeyMenu))},
			"KeyMinus":                   {Typ: reflect.TypeOf(q.KeyMinus), Value: constant.MakeInt64(int64(q.KeyMinus))},
			"KeyN":                       {Typ: reflect.TypeOf(q.KeyN), Value: constant.MakeInt64(int64(q.KeyN))},
			"KeyNumLock":                 {Typ: reflect.TypeOf(q.KeyNumLock), Value: constant.MakeInt64(int64(q.KeyNumLock))},
			"KeyO":                       {Typ: reflect.TypeOf(q.KeyO), Value: constant.MakeInt64(int64(q.KeyO))},
			"KeyP":                       {Typ: reflect.TypeOf(q.KeyP), Value: constant.MakeInt64(int64(q.KeyP))},
			"KeyPageDown":                {Typ: reflect.TypeOf(q.KeyPageDown), Value: constant.MakeInt64(int64(q.KeyPageDown))},
			"KeyPageUp":                  {Typ: reflect.TypeOf(q.KeyPageUp), Value: constant.MakeInt64(int64(q.KeyPageUp))},
			"KeyPause":                   {Typ: reflect.TypeOf(q.KeyPause), Value: constant.MakeInt64(int64(q.KeyPause))},
			"KeyPeriod":                  {Typ: reflect.TypeOf(q.KeyPeriod), Value: constant.MakeInt64(int64(q.KeyPeriod))},
			"KeyPrintScreen":             {Typ: reflect.TypeOf(q.KeyPrintScreen), Value: constant.MakeInt64(int64(q.KeyPrintScreen))},
			"KeyQ":                       {Typ: reflect.TypeOf(q.KeyQ), Value: constant.MakeInt64(int64(q.KeyQ))},
			"KeyR":                       {Typ: reflect.TypeOf(q.KeyR), Value: constant.MakeInt64(int64(q.KeyR))},
			"KeyRight":                   {Typ: reflect.TypeOf(q.KeyRight), Value: constant.MakeInt64(int64(q.KeyRight))},
			"KeyRightBracket":            {Typ: reflect.TypeOf(q.KeyRightBracket), Value: constant.MakeInt64(int64(q.KeyRightBracket))},
			"KeyS":                       {Typ: reflect.TypeOf(q.KeyS), Value: constant.MakeInt64(int64(q.KeyS))},
			"KeyScrollLock":              {Typ: reflect.TypeOf(q.KeyScrollLock), Value: constant.MakeInt64(int64(q.KeyScrollLock))},
			"KeySemicolon":               {Typ: reflect.TypeOf(q.KeySemicolon), Value: constant.MakeInt64(int64(q.KeySemicolon))},
			"KeyShift":                   {Typ: reflect.TypeOf(q.KeyShift), Value: constant.MakeInt64(int64(q.KeyShift))},
			"KeySlash":                   {Typ: reflect.TypeOf(q.KeySlash), Value: constant.MakeInt64(int64(q.KeySlash))},
			"KeySpace":                   {Typ: reflect.TypeOf(q.KeySpace), Value: constant.MakeInt64(int64(q.KeySpace))},
			"KeyT":                       {Typ: reflect.TypeOf(q.KeyT), Value: constant.MakeInt64(int64(q.KeyT))},
			"KeyTab":                     {Typ: reflect.TypeOf(q.KeyTab), Value: constant.MakeInt64(int64(q.KeyTab))},
			"KeyU":                       {Typ: reflect.TypeOf(q.KeyU), Value: constant.MakeInt64(int64(q.KeyU))},
			"KeyUp":                      {Typ: reflect.TypeOf(q.KeyUp), Value: constant.MakeInt64(int64(q.KeyUp))},
			"KeyV":                       {Typ: reflect.TypeOf(q.KeyV), Value: constant.MakeInt64(int64(q.KeyV))},
			"KeyW":                       {Typ: reflect.TypeOf(q.KeyW), Value: constant.MakeInt64(int64(q.KeyW))},
			"KeyX":                       {Typ: reflect.TypeOf(q.KeyX), Value: constant.MakeInt64(int64(q.KeyX))},
			"KeyY":                       {Typ: reflect.TypeOf(q.KeyY), Value: constant.MakeInt64(int64(q.KeyY))},
			"KeyZ":                       {Typ: reflect.TypeOf(q.KeyZ), Value: constant.MakeInt64(int64(q.KeyZ))},
			"KinematicPhysics":           {Typ: reflect.TypeOf(q.KinematicPhysics), Value: constant.MakeInt64(int64(q.KinematicPhysics))},
			"Last":                       {Typ: reflect.TypeOf(q.Last), Value: constant.MakeInt64(int64(q.Last))},
			"Left":                       {Typ: reflect.TypeOf(q.Left), Value: constant.MakeFromLiteral("-.9e+2", token.FLOAT, 0)},
			"LeftRight":                  {Typ: reflect.TypeOf(q.LeftRight), Value: constant.MakeInt64(int64(q.LeftRight))},
			"MOUSE_BUTTON_LEFT":          {Typ: reflect.TypeOf(q.MOUSE_BUTTON_LEFT), Value: constant.MakeInt64(int64(q.MOUSE_BUTTON_LEFT))},
			"MOUSE_BUTTON_MIDDLE":        {Typ: reflect.TypeOf(q.MOUSE_BUTTON_MIDDLE), Value: constant.MakeInt64(int64(q.MOUSE_BUTTON_MIDDLE))},
			"MOUSE_BUTTON_RIGHT":         {Typ: reflect.TypeOf(q.MOUSE_BUTTON_RIGHT), Value: constant.MakeInt64(int64(q.MOUSE_BUTTON_RIGHT))},
			"MosaicEffect":               {Typ: reflect.TypeOf(q.MosaicEffect), Value: constant.MakeInt64(int64(q.MosaicEffect))},
			"Mouse":                      {Typ: reflect.TypeOf(q.Mouse), Value: constant.MakeInt64(int64(q.Mouse))},
			"Next":                       {Typ: reflect.TypeOf(q.Next), Value: constant.MakeInt64(int64(q.Next))},
			"NoPhysics":                  {Typ: reflect.TypeOf(q.NoPhysics), Value: constant.MakeInt64(int64(q.NoPhysics))},
			"None":                       {Typ: reflect.TypeOf(q.None), Value: constant.MakeInt64(int64(q.None))},
			"Normal":                     {Typ: reflect.TypeOf(q.Normal), Value: constant.MakeInt64(int64(q.Normal))},
			"OtherScriptsInGame":         {Typ: reflect.TypeOf(q.OtherScriptsInGame), Value: constant.MakeInt64(int64(q.OtherScriptsInGame))},
			"OtherScriptsInSprite":       {Typ: reflect.TypeOf(q.OtherScriptsInSprite), Value: constant.MakeInt64(int64(q.OtherScriptsInSprite))},
			"PenBrightness":              {Typ: reflect.TypeOf(q.PenBrightness), Value: constant.MakeInt64(int64(q.PenBrightness))},
			"PenHue":                     {Typ: reflect.TypeOf(q.PenHue), Value: constant.MakeInt64(int64(q.PenHue))},
			"PenNone":                    {Typ: reflect.TypeOf(q.PenNone), Value: constant.MakeInt64(int64(q.PenNone))},
			"PenSaturation":              {Typ: reflect.TypeOf(q.PenSaturation), Value: constant.MakeInt64(int64(q.PenSaturation))},
			"PenTransparency":            {Typ: reflect.TypeOf(q.PenTransparency), Value: constant.MakeInt64(int64(q.PenTransparency))},
			"PixelateEffect":             {Typ: reflect.TypeOf(q.PixelateEffect), Value: constant.MakeInt64(int64(q.PixelateEffect))},
			"PolygonCollider":            {Typ: reflect.TypeOf(q.PolygonCollider), Value: constant.MakeInt64(int64(q.PolygonCollider))},
			"Prev":                       {Typ: reflect.TypeOf(q.Prev), Value: constant.MakeInt64(int64(q.Prev))},
			"Random":                     {Typ: reflect.TypeOf(q.Random), Value: constant.MakeInt64(int64(q.Random))},
			"RectCollider":               {Typ: reflect.TypeOf(q.RectCollider), Value: constant.MakeInt64(int64(q.RectCollider))},
			"Right":                      {Typ: reflect.TypeOf(q.Right), Value: constant.MakeFromLiteral("9.e+1", token.FLOAT, 0)},
			"SoundPanEffect":             {Typ: reflect.TypeOf(q.SoundPanEffect), Value: constant.MakeInt64(int64(q.SoundPanEffect))},
			"SoundPitchEffect":           {Typ: reflect.TypeOf(q.SoundPitchEffect), Value: constant.MakeInt64(int64(q.SoundPitchEffect))},
			"StateDie":                   {Typ: reflect.TypeOf(q.StateDie), Value: constant.MakeString(string(q.StateDie))},
			"StateGlide":                 {Typ: reflect.TypeOf(q.StateGlide), Value: constant.MakeString(string(q.StateGlide))},
			"StateStep":                  {Typ: reflect.TypeOf(q.StateStep), Value: constant.MakeString(string(q.StateStep))},
			"StateTurn":                  {Typ: reflect.TypeOf(q.StateTurn), Value: constant.MakeString(string(q.StateTurn))},
			"StaticPhysics":              {Typ: reflect.TypeOf(q.StaticPhysics), Value: constant.MakeInt64(int64(q.StaticPhysics))},
			"ThisScript":                 {Typ: reflect.TypeOf(q.ThisScript), Value: constant.MakeInt64(int64(q.ThisScript))},
			"ThisSprite":                 {Typ: reflect.TypeOf(q.ThisSprite), Value: constant.MakeInt64(int64(q.ThisSprite))},
			"TriggerExtraPixel":          {Typ: reflect.TypeOf(q.TriggerExtraPixel), Value: constant.MakeFromLiteral("2.e+0", token.FLOAT, 0)},
			"Up":                         {Typ: reflect.TypeOf(q.Up), Value: constant.MakeFromLiteral("0", token.FLOAT, 0)},
			"WhirlEffect":                {Typ: reflect.TypeOf(q.WhirlEffect), Value: constant.MakeInt64(int64(q.WhirlEffect))},
		},
		UntypedConsts: map[string]ixgo.UntypedConst{
			"All":                          {Typ: "untyped int", Value: constant.MakeInt64(int64(q.All))},
			"Gop_sched":                    {Typ: "untyped string", Value: constant.MakeString(string(q.Gop_sched))},
			"InputReplayFormat":            {Typ: "untyped string", Value: constant.MakeString(string(q.InputReplayFormat))},
			"InputReplayVersion":           {Typ: "untyped int", Value: constant.MakeInt64(int64(q.InputReplayVersion))},
			"XGoPackage":                   {Typ: "untyped bool", Value: constant.MakeBool(bool(q.XGoPackage))},
			"XGoo_SpriteImpl_GlideWith":    {Typ: "untyped string", Value: constant.MakeString(string(q.XGoo_SpriteImpl_GlideWith))},
			"XGoo_SpriteImpl_QuoteWith":    {Typ: "untyped string", Value: constant.MakeString(string(q.XGoo_SpriteImpl_QuoteWith))},
			"XGoo_SpriteImpl_SetLayerWith": {Typ: "untyped string", Value: constant.MakeString(string(q.XGoo_SpriteImpl_SetLayerWith))},
			"XGoo_SpriteImpl_StepToWith":   {Typ: "untyped string", Value: constant.MakeString(string(q.XGoo_SpriteImpl_StepToWith))},
			"XGoo_SpriteImpl_TurnToWith":   {Typ: "untyped string", Value: constant.MakeString(string(q.XGoo_SpriteImpl_TurnToWith))},
			"XGoo_Sprite_GlideWith":        {Typ: "untyped string", Value: constant.MakeString(string(q.XGoo_Sprite_GlideWith))},
			"XGoo_Sprite_QuoteWith":        {Typ: "untyped string", Value: constant.MakeString(string(q.XGoo_Sprite_QuoteWith))},
			"XGoo_Sprite_SetLayerWith":     {Typ: "untyped string", Value: constant.MakeString(string(q.XGoo_Sprite_SetLayerWith))},
			"XGoo_Sprite_StepToWith":       {Typ: "untyped string", Value: constant.MakeString(string(q.XGoo_Sprite_StepToWith))},
			"XGoo_Sprite_TurnToWith":       {Typ: "untyped string", Value: constant.MakeString(string(q.XGoo_Sprite_TurnToWith))},
			"XGou_Seconds":                 {Typ: "untyped string", Value: constant.MakeString(string(q.XGou_Seconds))},
		},
		Import: typesdata.ImportFunc("github.com/goplus/spx/v3", exportTypesData),
		Alias: map[string]alias.Type{
			"DeltaTime": &alias.Func{
				Params:  nil,
				Results: []alias.Type{alias_Seconds},
			},
			"Game": &alias.Named{
				Underlying: nil,
				Methods: map[string]*alias.Func{
					"WaitNextFrame": &alias.Func{
						Params:  nil,
						Results: []alias.Type{alias_Seconds},
					},
					"Wait": &alias.Func{
						Params:  []alias.Type{alias_Seconds},
						Results: nil,
					},
					"Timer": &alias.Func{
						Params:  nil,
						Results: []alias.Type{alias_Seconds},
					},
				},
			},
			"Sprite": &alias.Named{
				Underlying: &alias.Interface{
					Methods: map[string]*alias.Func{
						"GlideToTarget": &alias.Func{
							Params:  []alias.Type{nil, alias_Seconds},
							Results: nil,
						},
						"GlideToXYpos": &alias.Func{
							Params:  []alias.Type{nil, nil, alias_Seconds},
							Results: nil,
						},
						"Glide__0": &alias.Func{
							Params:  []alias.Type{nil, alias_Seconds},
							Results: nil,
						},
						"Glide__1": &alias.Func{
							Params:  []alias.Type{nil, alias_Seconds},
							Results: nil,
						},
						"Glide__2": &alias.Func{
							Params:  []alias.Type{nil, alias_Seconds},
							Results: nil,
						},
						"Glide__3": &alias.Func{
							Params:  []alias.Type{nil, alias_Seconds},
							Results: nil,
						},
						"Glide__4": &alias.Func{
							Params:  []alias.Type{nil, nil, alias_Seconds},
							Results: nil,
						},
						"QuoteMsg": &alias.Func{
							Params:  []alias.Type{nil, alias_Seconds},
							Results: nil,
						},
						"QuoteMsgEx": &alias.Func{
							Params:  []alias.Type{nil, nil, alias_Seconds},
							Results: nil,
						},
						"Quote__1": &alias.Func{
							Params:  []alias.Type{nil, alias_Seconds},
							Results: nil,
						},
						"Quote__3": &alias.Func{
							Params:  []alias.Type{nil, nil, alias_Seconds},
							Results: nil,
						},
						"SayWith": &alias.Func{
							Params:  []alias.Type{nil, alias_Seconds},
							Results: nil,
						},
						"Say__1": &alias.Func{
							Params:  []alias.Type{nil, alias_Seconds},
							Results: nil,
						},
						"ThinkWith": &alias.Func{
							Params:  []alias.Type{nil, alias_Seconds},
							Results: nil,
						},
						"Think__1": &alias.Func{
							Params:  []alias.Type{nil, alias_Seconds},
							Results: nil,
						},
					},
				},
			},
			"SpriteImpl": &alias.Named{
				Underlying: nil,
				Methods: map[string]*alias.Func{
					"Say__1": &alias.Func{
						Params:  []alias.Type{nil, alias_Seconds},
						Results: nil,
					},
					"SayWith": &alias.Func{
						Params:  []alias.Type{nil, alias_Seconds},
						Results: nil,
					},
					"Think__1": &alias.Func{
						Params:  []alias.Type{nil, alias_Seconds},
						Results: nil,
					},
					"ThinkWith": &alias.Func{
						Params:  []alias.Type{nil, alias_Seconds},
						Results: nil,
					},
					"Quote__1": &alias.Func{
						Params:  []alias.Type{nil, alias_Seconds},
						Results: nil,
					},
					"Quote__3": &alias.Func{
						Params:  []alias.Type{nil, nil, alias_Seconds},
						Results: nil,
					},
					"QuoteMsg": &alias.Func{
						Params:  []alias.Type{nil, alias_Seconds},
						Results: nil,
					},
					"QuoteMsgEx": &alias.Func{
						Params:  []alias.Type{nil, nil, alias_Seconds},
						Results: nil,
					},
					"Glide__0": &alias.Func{
						Params:  []alias.Type{nil, alias_Seconds},
						Results: nil,
					},
					"Glide__1": &alias.Func{
						Params:  []alias.Type{nil, alias_Seconds},
						Results: nil,
					},
					"Glide__2": &alias.Func{
						Params:  []alias.Type{nil, alias_Seconds},
						Results: nil,
					},
					"Glide__3": &alias.Func{
						Params:  []alias.Type{nil, alias_Seconds},
						Results: nil,
					},
					"Glide__4": &alias.Func{
						Params:  []alias.Type{nil, nil, alias_Seconds},
						Results: nil,
					},
					"GlideToTarget": &alias.Func{
						Params:  []alias.Type{nil, alias_Seconds},
						Results: nil,
					},
					"GlideToXYpos": &alias.Func{
						Params:  []alias.Type{nil, nil, alias_Seconds},
						Results: nil,
					},
				},
			},
			"TimeSinceLevelLoad": &alias.Func{
				Params:  nil,
				Results: []alias.Type{alias_Seconds},
			},
		},
	})
	ixgo.RegisterDirectCalls("github.com/goplus/spx/v3", map[string]ixgo.DirectCallAdapter{
		"(*Game).KeyPressed":           method_ptr_Game_KeyPressed,
		"(*Game).MousePressed":         method_ptr_Game_MousePressed,
		"(*Game).MouseX":               method_ptr_Game_MouseX,
		"(*Game).MouseY":               method_ptr_Game_MouseY,
		"(*Game).ResetTimer":           method_ptr_Game_ResetTimer,
		"(*Game).Stop":                 method_ptr_Game_Stop,
		"(*Game).Timer":                method_ptr_Game_Timer,
		"(*List).Append":               method_ptr_List_Append,
		"(*List).At":                   method_ptr_List_At,
		"(*List).Clear":                method_ptr_List_Clear,
		"(*List).Contains":             method_ptr_List_Contains,
		"(*List).Delete":               method_ptr_List_Delete,
		"(*List).IndexOf":              method_ptr_List_IndexOf,
		"(*List).InitFrom":             method_ptr_List_InitFrom,
		"(*List).Insert":               method_ptr_List_Insert,
		"(*List).Len":                  method_ptr_List_Len,
		"(*List).Set":                  method_ptr_List_Set,
		"(*List).String":               method_ptr_List_String,
		"(*SpriteImpl).ChangeHeading":  method_ptr_SpriteImpl_ChangeHeading,
		"(*SpriteImpl).ChangePenSize":  method_ptr_SpriteImpl_ChangePenSize,
		"(*SpriteImpl).ChangeXYpos":    method_ptr_SpriteImpl_ChangeXYpos,
		"(*SpriteImpl).ChangeXpos":     method_ptr_SpriteImpl_ChangeXpos,
		"(*SpriteImpl).ChangeYpos":     method_ptr_SpriteImpl_ChangeYpos,
		"(*SpriteImpl).CostumeIndex":   method_ptr_SpriteImpl_CostumeIndex,
		"(*SpriteImpl).Heading":        method_ptr_SpriteImpl_Heading,
		"(*SpriteImpl).Hide":           method_ptr_SpriteImpl_Hide,
		"(*SpriteImpl).Move__0":        method_ptr_SpriteImpl_Move__0,
		"(*SpriteImpl).Move__1":        method_ptr_SpriteImpl_Move__1,
		"(*SpriteImpl).PenDown":        method_ptr_SpriteImpl_PenDown,
		"(*SpriteImpl).PenUp":          method_ptr_SpriteImpl_PenUp,
		"(*SpriteImpl).SetCostume__0":  method_ptr_SpriteImpl_SetCostume__0,
		"(*SpriteImpl).SetCostume__1":  method_ptr_SpriteImpl_SetCostume__1,
		"(*SpriteImpl).SetCostume__2":  method_ptr_SpriteImpl_SetCostume__2,
		"(*SpriteImpl).SetHeading":     method_ptr_SpriteImpl_SetHeading,
		"(*SpriteImpl).SetPenColor__0": method_ptr_SpriteImpl_SetPenColor__0,
		"(*SpriteImpl).SetPenColor__1": method_ptr_SpriteImpl_SetPenColor__1,
		"(*SpriteImpl).SetPenSize":     method_ptr_SpriteImpl_SetPenSize,
		"(*SpriteImpl).SetXYpos":       method_ptr_SpriteImpl_SetXYpos,
		"(*SpriteImpl).SetXpos":        method_ptr_SpriteImpl_SetXpos,
		"(*SpriteImpl).SetYpos":        method_ptr_SpriteImpl_SetYpos,
		"(*SpriteImpl).Show":           method_ptr_SpriteImpl_Show,
		"(*SpriteImpl).Stop":           method_ptr_SpriteImpl_Stop,
		"(*SpriteImpl).Visible":        method_ptr_SpriteImpl_Visible,
		"(*SpriteImpl).Xpos":           method_ptr_SpriteImpl_Xpos,
		"(*SpriteImpl).Ypos":           method_ptr_SpriteImpl_Ypos,
		"(*Value).Equal":               method_ptr_Value_Equal,
		"(*Value).Float":               method_ptr_Value_Float,
		"(*Value).Int":                 method_ptr_Value_Int,
		"(*Value).Set":                 method_ptr_Value_Set,
		"(*Value).String":              method_ptr_Value_String,
		"(Value).Equal":                method_Value_Equal,
		"(Value).Float":                method_Value_Float,
		"(Value).Int":                  method_Value_Int,
		"(Value).String":               method_Value_String,
		"AtFrame":                      func_AtFrame,
		"CharAt":                       func_CharAt,
		"Compare":                      func_Compare,
		"Contains":                     func_Contains,
		"CurrentFrame":                 func_CurrentFrame,
		"DeltaTime":                    func_DeltaTime,
		"Equal":                        func_Equal,
		"Exit__0":                      func_Exit__0,
		"Exit__1":                      func_Exit__1,
		"FloorMod":                     func_FloorMod,
		"Forever":                      func_Forever,
		"GetInputSessionStatus":        func_GetInputSessionStatus,
		"GetWidget":                    func_GetWidget,
		"HSB":                          func_HSB,
		"HSBA":                         func_HSBA,
		"Iround":                       func_Iround,
		"IsRunWithoutScreenRefresh":    func_IsRunWithoutScreenRefresh,
		"KeyFromString":                func_KeyFromString,
		"NewColor":                     func_NewColor,
		"NewValue":                     func_NewValue,
		"PenColorParamFromString":      func_PenColorParamFromString,
		"Procedure":                    func_Procedure,
		"Rand__0":                      func_Rand__0,
		"Rand__1":                      func_Rand__1,
		"Repeat":                       func_Repeat,
		"RepeatUntil":                  func_RepeatUntil,
		"ResetRandomSeed":              func_ResetRandomSeed,
		"Sched":                        func_Sched,
		"SchedNow":                     func_SchedNow,
		"ScratchListIndex":             func_ScratchListIndex,
		"SetRandomSeed":                func_SetRandomSeed,
		"SetRunWithoutScreenRefresh":   func_SetRunWithoutScreenRefresh,
		"Snapshot":                     func_Snapshot,
		"TimeSinceLevelLoad":           func_TimeSinceLevelLoad,
		"WaitUntil":                    func_WaitUntil,
		"Warp":                         func_Warp,
		"XGot_Game_Main":               func_XGot_Game_Main,
		"XGot_Game_Reload":             func_XGot_Game_Reload,
	})
}

func func_AtFrame(ctx ixgo.DirectCallContext) {
	q.AtFrame(ixgo.DirectCallArg[int64](ctx, 0), ixgo.DirectCallArg[func()](ctx, 1))
}

func func_CharAt(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.CharAt(ixgo.DirectCallArg[string](ctx, 0), ixgo.DirectCallArg[int](ctx, 1)))
}

func func_Compare(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.Compare(ixgo.DirectCallArg[any](ctx, 0), ixgo.DirectCallArg[any](ctx, 1)))
}

func func_Contains(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.Contains(ixgo.DirectCallArg[string](ctx, 0), ixgo.DirectCallArg[string](ctx, 1)))
}

func func_CurrentFrame(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.CurrentFrame())
}

func func_DeltaTime(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.DeltaTime())
}

func func_Equal(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.Equal(ixgo.DirectCallArg[any](ctx, 0), ixgo.DirectCallArg[any](ctx, 1)))
}

func func_Exit__0(ctx ixgo.DirectCallContext) {
	q.Exit__0()
}

func func_Exit__1(ctx ixgo.DirectCallContext) {
	q.Exit__1(ixgo.DirectCallArg[int](ctx, 0))
}

func func_FloorMod(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.FloorMod(ixgo.DirectCallArg[float64](ctx, 0), ixgo.DirectCallArg[float64](ctx, 1)))
}

func func_Forever(ctx ixgo.DirectCallContext) {
	q.Forever(ixgo.DirectCallArg[func()](ctx, 0))
}

func method_ptr_Game_KeyPressed(ctx ixgo.DirectCallContext) {
	ctx.SetResult((*q.Game).KeyPressed(ixgo.DirectCallArg[*q.Game](ctx, 0), ixgo.DirectCallArg[engine.KeyCode](ctx, 1)))
}

func method_ptr_Game_MousePressed(ctx ixgo.DirectCallContext) {
	ctx.SetResult((*q.Game).MousePressed(ixgo.DirectCallArg[*q.Game](ctx, 0)))
}

func method_ptr_Game_MouseX(ctx ixgo.DirectCallContext) {
	ctx.SetResult((*q.Game).MouseX(ixgo.DirectCallArg[*q.Game](ctx, 0)))
}

func method_ptr_Game_MouseY(ctx ixgo.DirectCallContext) {
	ctx.SetResult((*q.Game).MouseY(ixgo.DirectCallArg[*q.Game](ctx, 0)))
}

func method_ptr_Game_ResetTimer(ctx ixgo.DirectCallContext) {
	(*q.Game).ResetTimer(ixgo.DirectCallArg[*q.Game](ctx, 0))
}

func method_ptr_Game_Stop(ctx ixgo.DirectCallContext) {
	(*q.Game).Stop(ixgo.DirectCallArg[*q.Game](ctx, 0), ixgo.DirectCallArg[event.StopKind](ctx, 1))
}

func method_ptr_Game_Timer(ctx ixgo.DirectCallContext) {
	ctx.SetResult((*q.Game).Timer(ixgo.DirectCallArg[*q.Game](ctx, 0)))
}

func func_GetInputSessionStatus(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.GetInputSessionStatus())
}

func func_GetWidget(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.GetWidget(ixgo.DirectCallArg[q.ShapeGetter](ctx, 0), ixgo.DirectCallArg[string](ctx, 1)))
}

func func_HSB(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.HSB(ixgo.DirectCallArg[float64](ctx, 0), ixgo.DirectCallArg[float64](ctx, 1), ixgo.DirectCallArg[float64](ctx, 2)))
}

func func_HSBA(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.HSBA(ixgo.DirectCallArg[float64](ctx, 0), ixgo.DirectCallArg[float64](ctx, 1), ixgo.DirectCallArg[float64](ctx, 2), ixgo.DirectCallArg[float64](ctx, 3)))
}

func func_Iround(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.Iround(ixgo.DirectCallArg[float64](ctx, 0)))
}

func func_IsRunWithoutScreenRefresh(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.IsRunWithoutScreenRefresh())
}

func func_KeyFromString(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.KeyFromString(ixgo.DirectCallArg[string](ctx, 0)))
}

func method_ptr_List_Append(ctx ixgo.DirectCallContext) {
	(*q.List).Append(ixgo.DirectCallArg[*q.List](ctx, 0), ixgo.DirectCallArg[any](ctx, 1))
}

func method_ptr_List_At(ctx ixgo.DirectCallContext) {
	ctx.SetResult((*q.List).At(ixgo.DirectCallArg[*q.List](ctx, 0), ixgo.DirectCallArg[int](ctx, 1)))
}

func method_ptr_List_Clear(ctx ixgo.DirectCallContext) {
	(*q.List).Clear(ixgo.DirectCallArg[*q.List](ctx, 0))
}

func method_ptr_List_Contains(ctx ixgo.DirectCallContext) {
	ctx.SetResult((*q.List).Contains(ixgo.DirectCallArg[*q.List](ctx, 0), ixgo.DirectCallArg[any](ctx, 1)))
}

func method_ptr_List_Delete(ctx ixgo.DirectCallContext) {
	(*q.List).Delete(ixgo.DirectCallArg[*q.List](ctx, 0), ixgo.DirectCallArg[int](ctx, 1))
}

func method_ptr_List_IndexOf(ctx ixgo.DirectCallContext) {
	ctx.SetResult((*q.List).IndexOf(ixgo.DirectCallArg[*q.List](ctx, 0), ixgo.DirectCallArg[any](ctx, 1)))
}

func method_ptr_List_InitFrom(ctx ixgo.DirectCallContext) {
	(*q.List).InitFrom(ixgo.DirectCallArg[*q.List](ctx, 0), ixgo.DirectCallArg[*q.List](ctx, 1))
}

func method_ptr_List_Insert(ctx ixgo.DirectCallContext) {
	(*q.List).Insert(ixgo.DirectCallArg[*q.List](ctx, 0), ixgo.DirectCallArg[int](ctx, 1), ixgo.DirectCallArg[any](ctx, 2))
}

func method_ptr_List_Len(ctx ixgo.DirectCallContext) {
	ctx.SetResult((*q.List).Len(ixgo.DirectCallArg[*q.List](ctx, 0)))
}

func method_ptr_List_Set(ctx ixgo.DirectCallContext) {
	(*q.List).Set(ixgo.DirectCallArg[*q.List](ctx, 0), ixgo.DirectCallArg[int](ctx, 1), ixgo.DirectCallArg[any](ctx, 2))
}

func method_ptr_List_String(ctx ixgo.DirectCallContext) {
	ctx.SetResult((*q.List).String(ixgo.DirectCallArg[*q.List](ctx, 0)))
}

func func_NewColor(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.NewColor(ixgo.DirectCallArg[any](ctx, 0)))
}

func func_NewValue(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.NewValue(ixgo.DirectCallArg[any](ctx, 0)))
}

func func_PenColorParamFromString(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.PenColorParamFromString(ixgo.DirectCallArg[string](ctx, 0)))
}

func func_Procedure(ctx ixgo.DirectCallContext) {
	q.Procedure(ixgo.DirectCallArg[func()](ctx, 0))
}

func func_Rand__0(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.Rand__0(ixgo.DirectCallArg[int](ctx, 0), ixgo.DirectCallArg[int](ctx, 1)))
}

func func_Rand__1(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.Rand__1(ixgo.DirectCallArg[float64](ctx, 0), ixgo.DirectCallArg[float64](ctx, 1)))
}

func func_Repeat(ctx ixgo.DirectCallContext) {
	q.Repeat(ixgo.DirectCallArg[int](ctx, 0), ixgo.DirectCallArg[func()](ctx, 1))
}

func func_RepeatUntil(ctx ixgo.DirectCallContext) {
	q.RepeatUntil(ixgo.DirectCallArg[func() bool](ctx, 0), ixgo.DirectCallArg[func()](ctx, 1))
}

func func_ResetRandomSeed(ctx ixgo.DirectCallContext) {
	q.ResetRandomSeed()
}

func func_Sched(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.Sched())
}

func func_SchedNow(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.SchedNow())
}

func func_ScratchListIndex(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.ScratchListIndex(ixgo.DirectCallArg[any](ctx, 0)))
}

func func_SetRandomSeed(ctx ixgo.DirectCallContext) {
	q.SetRandomSeed(ixgo.DirectCallArg[int64](ctx, 0))
}

func func_SetRunWithoutScreenRefresh(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.SetRunWithoutScreenRefresh(ixgo.DirectCallArg[bool](ctx, 0)))
}

func func_Snapshot(ctx ixgo.DirectCallContext) {
	q.Snapshot(ixgo.DirectCallArg[string](ctx, 0), ixgo.DirectCallArg[func() error](ctx, 1))
}

func method_ptr_SpriteImpl_ChangeHeading(ctx ixgo.DirectCallContext) {
	(*q.SpriteImpl).ChangeHeading(ixgo.DirectCallArg[*q.SpriteImpl](ctx, 0), ixgo.DirectCallArg[float64](ctx, 1))
}

func method_ptr_SpriteImpl_ChangePenSize(ctx ixgo.DirectCallContext) {
	(*q.SpriteImpl).ChangePenSize(ixgo.DirectCallArg[*q.SpriteImpl](ctx, 0), ixgo.DirectCallArg[float64](ctx, 1))
}

func method_ptr_SpriteImpl_ChangeXYpos(ctx ixgo.DirectCallContext) {
	(*q.SpriteImpl).ChangeXYpos(ixgo.DirectCallArg[*q.SpriteImpl](ctx, 0), ixgo.DirectCallArg[float64](ctx, 1), ixgo.DirectCallArg[float64](ctx, 2))
}

func method_ptr_SpriteImpl_ChangeXpos(ctx ixgo.DirectCallContext) {
	(*q.SpriteImpl).ChangeXpos(ixgo.DirectCallArg[*q.SpriteImpl](ctx, 0), ixgo.DirectCallArg[float64](ctx, 1))
}

func method_ptr_SpriteImpl_ChangeYpos(ctx ixgo.DirectCallContext) {
	(*q.SpriteImpl).ChangeYpos(ixgo.DirectCallArg[*q.SpriteImpl](ctx, 0), ixgo.DirectCallArg[float64](ctx, 1))
}

func method_ptr_SpriteImpl_CostumeIndex(ctx ixgo.DirectCallContext) {
	ctx.SetResult((*q.SpriteImpl).CostumeIndex(ixgo.DirectCallArg[*q.SpriteImpl](ctx, 0)))
}

func method_ptr_SpriteImpl_Heading(ctx ixgo.DirectCallContext) {
	ctx.SetResult((*q.SpriteImpl).Heading(ixgo.DirectCallArg[*q.SpriteImpl](ctx, 0)))
}

func method_ptr_SpriteImpl_Hide(ctx ixgo.DirectCallContext) {
	(*q.SpriteImpl).Hide(ixgo.DirectCallArg[*q.SpriteImpl](ctx, 0))
}

func method_ptr_SpriteImpl_Move__0(ctx ixgo.DirectCallContext) {
	(*q.SpriteImpl).Move__0(ixgo.DirectCallArg[*q.SpriteImpl](ctx, 0), ixgo.DirectCallArg[float64](ctx, 1))
}

func method_ptr_SpriteImpl_Move__1(ctx ixgo.DirectCallContext) {
	(*q.SpriteImpl).Move__1(ixgo.DirectCallArg[*q.SpriteImpl](ctx, 0), ixgo.DirectCallArg[int](ctx, 1))
}

func method_ptr_SpriteImpl_PenDown(ctx ixgo.DirectCallContext) {
	(*q.SpriteImpl).PenDown(ixgo.DirectCallArg[*q.SpriteImpl](ctx, 0))
}

func method_ptr_SpriteImpl_PenUp(ctx ixgo.DirectCallContext) {
	(*q.SpriteImpl).PenUp(ixgo.DirectCallArg[*q.SpriteImpl](ctx, 0))
}

func method_ptr_SpriteImpl_SetCostume__0(ctx ixgo.DirectCallContext) {
	(*q.SpriteImpl).SetCostume__0(ixgo.DirectCallArg[*q.SpriteImpl](ctx, 0), ixgo.DirectCallArg[string](ctx, 1))
}

func method_ptr_SpriteImpl_SetCostume__1(ctx ixgo.DirectCallContext) {
	(*q.SpriteImpl).SetCostume__1(ixgo.DirectCallArg[*q.SpriteImpl](ctx, 0), ixgo.DirectCallArg[float64](ctx, 1))
}

func method_ptr_SpriteImpl_SetCostume__2(ctx ixgo.DirectCallContext) {
	(*q.SpriteImpl).SetCostume__2(ixgo.DirectCallArg[*q.SpriteImpl](ctx, 0), ixgo.DirectCallArg[int](ctx, 1))
}

func method_ptr_SpriteImpl_SetHeading(ctx ixgo.DirectCallContext) {
	(*q.SpriteImpl).SetHeading(ixgo.DirectCallArg[*q.SpriteImpl](ctx, 0), ixgo.DirectCallArg[float64](ctx, 1))
}

func method_ptr_SpriteImpl_SetPenColor__0(ctx ixgo.DirectCallContext) {
	(*q.SpriteImpl).SetPenColor__0(ixgo.DirectCallArg[*q.SpriteImpl](ctx, 0), ixgo.DirectCallArg[q.Color](ctx, 1))
}

func method_ptr_SpriteImpl_SetPenColor__1(ctx ixgo.DirectCallContext) {
	(*q.SpriteImpl).SetPenColor__1(ixgo.DirectCallArg[*q.SpriteImpl](ctx, 0), ixgo.DirectCallArg[q.PenColorParam](ctx, 1), ixgo.DirectCallArg[float64](ctx, 2))
}

func method_ptr_SpriteImpl_SetPenSize(ctx ixgo.DirectCallContext) {
	(*q.SpriteImpl).SetPenSize(ixgo.DirectCallArg[*q.SpriteImpl](ctx, 0), ixgo.DirectCallArg[float64](ctx, 1))
}

func method_ptr_SpriteImpl_SetXYpos(ctx ixgo.DirectCallContext) {
	(*q.SpriteImpl).SetXYpos(ixgo.DirectCallArg[*q.SpriteImpl](ctx, 0), ixgo.DirectCallArg[float64](ctx, 1), ixgo.DirectCallArg[float64](ctx, 2))
}

func method_ptr_SpriteImpl_SetXpos(ctx ixgo.DirectCallContext) {
	(*q.SpriteImpl).SetXpos(ixgo.DirectCallArg[*q.SpriteImpl](ctx, 0), ixgo.DirectCallArg[float64](ctx, 1))
}

func method_ptr_SpriteImpl_SetYpos(ctx ixgo.DirectCallContext) {
	(*q.SpriteImpl).SetYpos(ixgo.DirectCallArg[*q.SpriteImpl](ctx, 0), ixgo.DirectCallArg[float64](ctx, 1))
}

func method_ptr_SpriteImpl_Show(ctx ixgo.DirectCallContext) {
	(*q.SpriteImpl).Show(ixgo.DirectCallArg[*q.SpriteImpl](ctx, 0))
}

func method_ptr_SpriteImpl_Stop(ctx ixgo.DirectCallContext) {
	(*q.SpriteImpl).Stop(ixgo.DirectCallArg[*q.SpriteImpl](ctx, 0), ixgo.DirectCallArg[event.StopKind](ctx, 1))
}

func method_ptr_SpriteImpl_Visible(ctx ixgo.DirectCallContext) {
	ctx.SetResult((*q.SpriteImpl).Visible(ixgo.DirectCallArg[*q.SpriteImpl](ctx, 0)))
}

func method_ptr_SpriteImpl_Xpos(ctx ixgo.DirectCallContext) {
	ctx.SetResult((*q.SpriteImpl).Xpos(ixgo.DirectCallArg[*q.SpriteImpl](ctx, 0)))
}

func method_ptr_SpriteImpl_Ypos(ctx ixgo.DirectCallContext) {
	ctx.SetResult((*q.SpriteImpl).Ypos(ixgo.DirectCallArg[*q.SpriteImpl](ctx, 0)))
}

func func_TimeSinceLevelLoad(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.TimeSinceLevelLoad())
}

func method_Value_Equal(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.Value.Equal(ixgo.DirectCallArg[q.Value](ctx, 0), ixgo.DirectCallArg[any](ctx, 1)))
}

func method_ptr_Value_Equal(ctx ixgo.DirectCallContext) {
	ctx.SetResult((*q.Value).Equal(ixgo.DirectCallArg[*q.Value](ctx, 0), ixgo.DirectCallArg[any](ctx, 1)))
}

func method_Value_Float(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.Value.Float(ixgo.DirectCallArg[q.Value](ctx, 0)))
}

func method_ptr_Value_Float(ctx ixgo.DirectCallContext) {
	ctx.SetResult((*q.Value).Float(ixgo.DirectCallArg[*q.Value](ctx, 0)))
}

func method_Value_Int(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.Value.Int(ixgo.DirectCallArg[q.Value](ctx, 0)))
}

func method_ptr_Value_Int(ctx ixgo.DirectCallContext) {
	ctx.SetResult((*q.Value).Int(ixgo.DirectCallArg[*q.Value](ctx, 0)))
}

func method_ptr_Value_Set(ctx ixgo.DirectCallContext) {
	(*q.Value).Set(ixgo.DirectCallArg[*q.Value](ctx, 0), ixgo.DirectCallArg[any](ctx, 1))
}

func method_Value_String(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.Value.String(ixgo.DirectCallArg[q.Value](ctx, 0)))
}

func method_ptr_Value_String(ctx ixgo.DirectCallContext) {
	ctx.SetResult((*q.Value).String(ixgo.DirectCallArg[*q.Value](ctx, 0)))
}

func func_WaitUntil(ctx ixgo.DirectCallContext) {
	q.WaitUntil(ixgo.DirectCallArg[func() bool](ctx, 0))
}

func func_Warp(ctx ixgo.DirectCallContext) {
	q.Warp(ixgo.DirectCallArg[func()](ctx, 0))
}

func func_XGot_Game_Main(ctx ixgo.DirectCallContext) {
	q.XGot_Game_Main(ixgo.DirectCallArg[q.Gamer](ctx, 0), ixgo.DirectCallArg[[]q.Sprite](ctx, 1)...)
}

func func_XGot_Game_Reload(ctx ixgo.DirectCallContext) {
	ctx.SetResult(q.XGot_Game_Reload(ixgo.DirectCallArg[q.Gamer](ctx, 0), ixgo.DirectCallArg[any](ctx, 1)))
}
